...
def get_graphene_hubbard(U = 0, size = 20):
    ...
    shape = Rhomboid(size,size,armchair = False)
...
def get_emitter_positions(flake, points=1000):
    y_min=jnp.min(flake.positions[:,1])
    y_idxs=jnp.where(jnp.abs(flake.positions[:,1]-y_min)<=1e-4)[0]
    llc = y_idxs[0]
    lrc=y_idxs[-1]
    mid_point=(flake.positions[llc]+flake.positions[lrc])/2
    ...
    line=jnp.vstack((extended_point.T, line))
...
# =====================================================Execution Code=========================================#
#
# =========Plot the Rhombus flake and the path ==============#
...
if __name__ == '__main__':
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.savefig("Rhombus_structure.png")
    ...
    jnp.savez(f"Rhomb{size}.npz", img=jnp.array(img_path), omega = omega)
    ...
    for coulomb, polarization, component in product(range(n_coulomb), range(n_polarization), range(n_component)):
        ...
        title = f"Rhomb{size}_U{coulomb*2}, P_{index[polarization]}, E_{index[component]}"
        plt.title(title)
        ...
        # Save figure
        filename = f"Rhomb{size}_U{coulomb}_P{index[polarization]}_E{index[component]}.png"
