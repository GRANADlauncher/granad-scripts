...
def get_emitter_positions(flake, points=12):
    # lower left corner => min x, then min y
    x_min = flake.positions[:, 0].min()
    x_idxs = jnp.where(jnp.abs(flake.positions[:, 0] - x_min) <= 1e-4)
    y_idx = jnp.argmin(flake.positions[x_idxs, 1], axis = -1)
    llc = x_idxs[0][y_idx[0]]
    llc_pos = flake.positions[llc]
    
    # center lower left corner => min y, then min x
    y_min = flake.positions[:, 1].min()
    y_idxs = jnp.where(jnp.abs(flake.positions[:, 1] - y_min) <= 1e-4)
    x_idx = jnp.argmin(flake.positions[y_idxs, 0], axis = -1)
    clc = y_idxs[0][x_idx[0]]
    clc_pos = flake.positions[clc]
    
    # scan over positions from triangle midpoint - flake center 
    mid_point = (clc_pos + llc_pos)/2
    ...
    emitter_positions=find_atoms_along_path(flake.positions,line)

    return emitter_positions, line
...
# =====================================================Execution Code=========================================#
#
# =========Plot the Hexagonal flake and the path ==============#
...
if __name__ == '__main__':
    ...
    jnp.savez(f"Hex{size}.npz", img=jnp.array(img_path), omega = omega)
    ...
    for coulomb, polarization, component in product(range(n_coulomb), range(n_polarization), range(n_component)):
        ...
        title = f"Hex{size}_U{coulomb*2}, P_{index[polarization]}, E_{index[component]}"
        plt.title(title)
        ...
        # Save figure
        filename = f"Hex{size}_U{coulomb}_P{index[polarization]}_E{index[component]}.png"
